...
const handleSetUserTrains = async () => {
  try {
    ...
    for (let page = 0; page < totalPages; page++) {
      const pageResponse = await fetch(`http://212.41.6.237/api/trains?search=author:${author}&page=${page}`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const fetchUserImage = async (userId) => {
    ...
    try {
      const response = await fetch(`http://212.41.6.237/api/users/${userId}/image`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const uploadImage = async (userId) => {
  if (isFileSelected === false) {
    ...
    console.log(`http://212.41.6.237/api/users/${userId}/image`)
    try {
        console.log(`http://212.41.6.237/api/users/${userId}/image`)
        const response = await fetch(`http://212.41.6.237/api/users/${userId}/image`, {
            method: 'POST',
